    ...
    <div className="bucket-modal-inner">
                <div className="bucket-modal-head">
                    ...
                    <div className="bucket-counter bkt-head-content">
                        <button className="btn btn-danger btn-cart-counter" type="submit">{cart.totalQuantity}</button>
                    </div>
                        ...
                        {cart.items.map(item => (
                          <div className="bucket-summary" key={item.id}>
                              <div className="item-detail">
                                  ...
                                  <button type="button" className="btn btn-outline-danger" onClick={() => handleIncrement(item)}>+</button>
                                  <button type="button" className="btn btn-outline-danger" onClick={() => handleDecrement(item)}>-</button>
                              </div>
                              <div className="price-detail">
                                  <h5>₫{(item.price * item.quantity).toLocaleString('vi-VN')}</h5>
                              </div>
                          </div>
                        ))}
                        <div className="grand-total">
                            <h5>TỔNG CỘNG</h5>
                            <h5>₫{cart.totalPrice.toLocaleString('vi-VN')}</h5>
                        </div>
                      </>
                    ) : (
